...
export default function Dashboard(){
  ...
  useEffect(()=>{load()},[])
  async function deleteListing(id:string){ if(!confirm('Delete this listing? This cannot be undone.')) return; const {error}=await supabase.from('tutor_listings').delete().eq('id',id); if(!error) setItems(items.filter(x=>x.id!==id)) }
  return <main className="dash">
    <div className="dash-welcome"><p className="eyebrow">YOUR TUITIONSETU ACCOUNT</p><h1>Hello{name?`, ${name}`:''} 👋</h1></div>
    {role==='tutor' && <div className="dash-section"><div className="dash-section-head"><h2>Your listings</h2><a className="primary inline" href="/tutor/new">+ Add listing</a></div>{items.length ? <div className="status-list">{items.map(x=><div className="dash-card" key={x.id}><div style={{display:'flex',alignItems:'center',gap:14}}><div className="dash-icon">🎓</div><div><b>{x.title}</b><small>{x.category} · {x.city}</small></div></div><div style={{display:'flex',alignItems:'center',gap:10}}><span className={`badge-pill ${x.status}`}>{x.status}</span><a className="link-btn" href={`/tutor/${x.id}`}>View</a><a className="link-btn" href={`/tutor/edit/${x.id}`}>Edit</a><button className="link-btn" onClick={()=>deleteListing(x.id)}>Delete</button></div></div>)}</div> : <div className="dash-empty">You haven't created a listing yet. <a href="/tutor/new">Add one now →</a></div>}</div>}
